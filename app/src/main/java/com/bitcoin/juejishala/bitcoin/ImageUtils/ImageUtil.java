...
public class ImageUtil {
    ...
    public static void runOnUIThread(Runnable runnable) {
        ImageLoaderConfig.getMainHandler().post(runnable);
    }
    ...
    public static boolean shouldSetPlaceHolder(SingleConfig config) {
        ...
        if (config.getResId() > 0 || !TextUtils.isEmpty(config.getFilePath()) || ImageLoaderConfig.getLoader().isCached(config.getUrl())) {
            return false;
    ...
    public static int dip2px(float dipValue) {
        final float scale = ImageLoaderConfig.context.getResources().getDisplayMetrics().density;
    ...
    public static String appendUrl(String url) {
        ...
        if (!hasHost) {
            if (!TextUtils.isEmpty(ImageLoaderConfig.baseUrl)) {
                newUrl = ImageLoaderConfig.baseUrl + url;
            }
        }
