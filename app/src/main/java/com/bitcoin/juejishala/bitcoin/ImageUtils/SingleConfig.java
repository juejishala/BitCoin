...
class SingleConfig {
    ...
    public SingleConfig(ConfigBuilder builder) {
        ...
        this.brightnessLeve = builder.brightnessLevel; //是否需要亮度
        this.filteColor = builder.filterColor;
        this.needBlur = builder.needBlur;
        this.needFilteColor = builder.needFilterColor;
    ...
    public Context getContext() {
        if (context == null) {
            context = ImageLoaderConfig.context;
        }
        return context;
    }
    ...
    public int getHeight() {
        if (height <= 0) {
            ...
            if (height <= 0) {
                height = ImageLoaderConfig.getWinHeight();
            }
        }
        return height;
    }
    ...
    public int getWidth() {
        if (width <= 0) {
            ...
            if (width <= 0) {
                width =ImageLoaderConfig.getWinWidth();
            }
        }
    ...
    private void show() {
        ImageLoaderConfig.getLoader().request(this);
    }
    ...
    public static class ConfigBuilder {
        ...
        private boolean ignoreCertificateVerify = ImageLoaderConfig.ignoreCertificateVerify;
        ...
        private float brightnessLevel; //亮度等级
        private boolean needBlur = false;//是否需要模糊
        private boolean needFilterColor = false;//是否需要滤镜颜色
        private int blurRadius;
        ...
        public ConfigBuilder colorFilter(int filteColor) {
            this.filterColor = filteColor;
            this.needFilterColor = true;
            return this;
        }
